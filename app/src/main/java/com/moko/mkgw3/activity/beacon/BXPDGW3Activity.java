...
public class BXPDGW3Activity extends BaseActivity<ActivityBxpDInfoKgw3Binding> {
    ...
    @Override
    protected ActivityBxpDInfoKgw3Binding getViewBinding() {
        return ActivityBxpDInfoKgw3Binding.inflate(getLayoutInflater());
    }
    ...
    private void gotoAccParams() {
        if (isWindowLocked()) return;
        Intent intent = new Intent(this, BXPDAccParamsGW3Activity.class);
        intent.putExtra(AppConstants.EXTRA_KEY_DEVICE, mMokoDeviceKgw3);
        intent.putExtra(AppConstants.EXTRA_KEY_MAC, mBeaconInfo.mac);
        startActivity(intent);
    }
