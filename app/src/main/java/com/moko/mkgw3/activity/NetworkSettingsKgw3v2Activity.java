...
public class NetworkSettingsKgw3v2Activity extends BaseActivity<ActivityNetworkSettingsKgw3v2Binding> {
    ...
    protected void onCreate() {
        ...
        mBind.wifiDhcp.imgDhcp.setOnClickListener(v -> {
            if (selectedNetworkType == 0) {
                ...
                setDhcpEnable(mBind.wifiDhcp, ethernetDhcpEnable);
            } else {
    ...
    private void onNetworkTypeClick() {
        ...
        dialog.setListener(value -> {
            ...
            //更新开关状态
            setDhcpEnable(mBind.wifiDhcp, value == 0 ? ethernetDhcpEnable : wifiDhcpEnable);
            setIpInfo(mBind.eltDhcp);
        });
                            ...
                            switch (configKeyEnum) {
                                ...
                                case KEY_ETHERNET_DHCP:
                                    ...
                                    if (selectedNetworkType != 1)
                                        setDhcpEnable(mBind.eltDhcp, ethernetDhcpEnable);
                                    break;
                                ...
                                case KEY_ETHERNET_IP_INFO:
                                    ...
                                    if (selectedNetworkType != 1) setIpInfo(mBind.eltDhcp);
                                    break;
                            }
                        }
                    }
    ...
    private boolean isParaError() {
        ...
        if (selectedNetworkType != 1) {
            if (!ethernetDhcpEnable) {
                //检查ip地址是否合法
                String ip = mBind.eltDhcp.etIp.getText().toString();
                String mask = mBind.eltDhcp.etMask.getText().toString();
                String gateway = mBind.eltDhcp.etGateway.getText().toString();
                String dns = mBind.eltDhcp.etDns.getText().toString();
        ...
        try {
            ...
            if (selectedNetworkType > 0) {
                ...
                if (selectedNetworkType > 1) {
                    ...
                    if (!ethernetDhcpEnable) {
                        String[] ipInfo = getDHCPInfo(mBind.eltDhcp);
