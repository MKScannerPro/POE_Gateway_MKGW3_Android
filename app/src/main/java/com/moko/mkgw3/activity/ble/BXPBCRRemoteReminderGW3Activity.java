...
import com.moko.mkgw3.databinding.ActivityBxpBCrRemoteReminderKgw3Binding;
...
public class BXPBCRRemoteReminderGW3Activity extends BaseActivity<ActivityBxpBCrRemoteReminderKgw3Binding> {
    ...
    @Override
    protected ActivityBxpBCrRemoteReminderKgw3Binding getViewBinding() {
        return ActivityBxpBCrRemoteReminderKgw3Binding.inflate(getLayoutInflater());
    }
    ...
    private void setVibrationNotifyRemind() {
        ...
        jsonObject.addProperty("shake_interval", buzzerInterval);
        String message = assembleWriteCommonData(msgId, mMokoDeviceKgw3.mac, jsonObject);
        try {
            MQTTSupport.getInstance().publish(mAppTopic, message, msgId, appMqttConfig.qos);
        } catch (MqttException e) {
    ...
    private boolean isVibrationValid() {
        ...
        if (shakeInterval < 0 || shakeInterval > 100)
            return false;
        return true;
    }
