...
public class ModifyMQTTSettingsKgw3Activity extends BaseActivity<ActivityMqttDeviceModifyKgw3Binding> implements RadioGroup.OnCheckedChangeListener {
    ...
    @Override
    protected ActivityMqttDeviceModifyKgw3Binding getViewBinding() {
        return ActivityMqttDeviceModifyKgw3Binding.inflate(getLayoutInflater());
    }
...
//                    row17.createCell(1).setCellValue("");
                ...
                runOnUiThread(() -> {
                    ...
                    if (isFileError) {
                        isFileError = false;
                        ToastUtils.showToast(ModifyMQTTSettingsKgw3Activity.this, "Export error!");
                        return;
                    }
                    ToastUtils.showToast(ModifyMQTTSettingsKgw3Activity.this, "Export success!");
                    Utils.sendEmail(ModifyMQTTSettingsKgw3Activity.this, "", "", "Settings for Device", "Choose Email Client", expertFile);
                    ...
                    new Thread(() -> {
                        try {
                            ...
                            if (rows < 18 || columns < 3) {
                                runOnUiThread(() -> {
                                    dismissLoadingProgressDialog();
                                    ToastUtils.showToast(ModifyMQTTSettingsKgw3Activity.this, "Please select the correct file!");
                                });
                                return;
                            }
                            ...
                            runOnUiThread(() -> {
                                dismissLoadingProgressDialog();
                                if (isFileError) {
                                    ToastUtils.showToast(ModifyMQTTSettingsKgw3Activity.this, "Import failed!");
                                    return;
                                }
                                ToastUtils.showToast(ModifyMQTTSettingsKgw3Activity.this, "Import success!");
                                initData();
                            });
                        } catch (Exception e) {
                            e.printStackTrace();
