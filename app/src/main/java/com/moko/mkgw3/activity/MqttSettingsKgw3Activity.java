...
public class MqttSettingsKgw3Activity extends BaseActivity<ActivityMqttDeviceKgw3Binding> implements RadioGroup.OnCheckedChangeListener {
    ...
    @Override
    protected ActivityMqttDeviceKgw3Binding getViewBinding() {
        return ActivityMqttDeviceKgw3Binding.inflate(getLayoutInflater());
    }
...
//                    row17.createCell(1).setCellValue("");
                ...
                runOnUiThread(() -> {
                    ...
                    if (isFileError) {
                        isFileError = false;
                        ToastUtils.showToast(MqttSettingsKgw3Activity.this, "Export error!");
                        return;
                    }
                    ToastUtils.showToast(MqttSettingsKgw3Activity.this, "Export success!");
                    Utils.sendEmail(MqttSettingsKgw3Activity.this, "", "", "Settings for Device", "Choose Email Client", expertFile);
                    ...
                    new Thread(() -> {
                        try {
                            ...
                            if (rows < 18 || columns < 3) {
                                runOnUiThread(() -> {
                                    dismissLoadingProgressDialog();
                                    ToastUtils.showToast(MqttSettingsKgw3Activity.this, "Please select the correct file!");
                                });
                                return;
                            }
                            ...
                            runOnUiThread(() -> {
                                dismissLoadingProgressDialog();
                                if (isFileError) {
                                    ToastUtils.showToast(MqttSettingsKgw3Activity.this, "Import failed!");
                                    return;
                                }
                                ToastUtils.showToast(MqttSettingsKgw3Activity.this, "Import success!");
                                initData();
                            });
                        } catch (Exception e) {
                            e.printStackTrace();
