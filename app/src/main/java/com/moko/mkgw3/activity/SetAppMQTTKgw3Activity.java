...
import com.moko.support.mkgw3.event.MQTTConnectionFailureEvent;
...
public class SetAppMQTTKgw3Activity extends BaseActivity<ActivityMqttAppKgw3Binding> implements RadioGroup.OnCheckedChangeListener {
    ...
    protected void onCreate() {
        ...
        expertFilePath = MKGW3MainActivity.PATH_LOGCAT + File.separator + "export" + File.separator + "Settings for APP.xlsx";
    }

    @Override
    protected ActivityMqttAppKgw3Binding getViewBinding() {
        return ActivityMqttAppKgw3Binding.inflate(getLayoutInflater());
    }
    ...
    public void onMQTTConnectionFailureEvent(MQTTConnectionFailureEvent event) {
        ToastUtils.showToast(SetAppMQTTKgw3Activity.this, getString(R.string.mqtt_connect_failed));
        dismissLoadingProgressDialog();
        finish();
    }
...
//                    row10.createCell(1).setCellValue("");
                ...
                runOnUiThread(() -> {
                    ...
                    if (isFileError) {
                        isFileError = false;
                        ToastUtils.showToast(SetAppMQTTKgw3Activity.this, "Export error!");
                        return;
                    }
                    ToastUtils.showToast(SetAppMQTTKgw3Activity.this, "Export success!");
                    Utils.sendEmail(SetAppMQTTKgw3Activity.this, "", "", "Settings for APP", "Choose Email Client", expertFile);
                    ...
                    new Thread(() -> {
                        try {
                            ...
                            if (rows < 13 || columns < 3) {
                                runOnUiThread(() -> {
                                    dismissLoadingProgressDialog();
                                    ToastUtils.showToast(SetAppMQTTKgw3Activity.this, "Please select the correct file!");
                                });
                                return;
                            }
                            ...
                            runOnUiThread(() -> {
                                dismissLoadingProgressDialog();
                                if (isFileError) {
                                    ToastUtils.showToast(SetAppMQTTKgw3Activity.this, "Import failed!");
                                    return;
                                }
                                ToastUtils.showToast(SetAppMQTTKgw3Activity.this, "Import success!");
                                initData();
                            });
                        } catch (Exception e) {
                            e.printStackTrace();
